...
「CLAUDE.mdを更新して」「マスタードキュメントを更新して」等の更新指示を検出し、対象ファイルをバックアップしてから用途別の更新コンテキストを注入する。

**発火条件:**
- プロンプトに `CLAUDE.md` / `マスタードキュメント` / 引用パス (`"path/to/file.md"`) / 省略形 `ドキュメント` のいずれか
- かつ「更新」「追記」「記載」「追加」「反映」「修正」等のアクション語
- `cwd` が存在し `stop_hook_active=false` のときのみ発火

**4モード分岐（注入コンテキストが異なる）:**
| モード | 発火プロンプト例 | 注入コンテキストの特徴 |
|--------|-----------------|-----------------------|
| `claude` | `CLAUDE.md を更新して` | 70% 縮小・整合性チェック・古い情報削除つき更新コンテキスト |
| `claude_append` | `CLAUDE.md に追記して` | 追記専用。縮小・書き換えなし |
| `master` | `マスタードキュメントを更新して` | 既存フォーマット厳守の進捗更新コンテキスト |
| `doc_smart` | `ドキュメントを更新して`（対象未指定） | 候補 `.md` を列挙して Claude に選ばせる |

**挙動:**
- 対象ファイルを `.bak` としてバックアップ（`doc_smart` は Claude 側でバックアップ作成）
- Claude へ「Append-only / 品質チェックリスト / 履歴書き込み」を含む `additionalContext` を返す
- 「グローバル CLAUDE.md」指定は `global-claude-md-appender.py` に処理を譲る
