...
CI では **Python 3.10 / 3.11 / 3.12** で `pytest tests/ -xvs` を回している。  
fresh fork 直後も、まずは同じ validation をローカルで通せるか確認するのがおすすめ。
...
テスト skill は **directory を作るところまで** を担当し、`SKILL.md` 本体の配布・配置は別途必要。
settings.jsonへの登録は手動で行う（以下を参照）。

つまり `install.ps1` 直後に確認できるのは:

1. hook ファイルが `~/.claude/hooks/` に入る
2. `settings.json` に手動登録できる
3. local validation (`pytest tests/ -xvs`) が通る

であり、smart test dispatch の完全再現には別途 skill 本体が必要。
...
ローカルで最初に confidence を作るなら、このテストを 1 回通してから hook block を試すのが分かりやすい。
